...
def _event_time_prefix_rate(result: dict[str, Any] | None) -> float:
    ...
    for row in metrics:
        ...
        scored.append({
            **row,
            ...
            "time_prefix_rate": round(_event_time_prefix_rate(result), 4),
...
def aggregate_rows(scored: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for (backend, model_name, media_type, duration_bucket), rows in sorted(groups.items()):
        ...
        summary.append({
            ...
            "avg_time_prefix_rate": round(_avg([float(row.get("time_prefix_rate", 0.0)) for row in rows]), 4),
...
def run(args: argparse.Namespace) -> int:
    metrics_path = _resolve_path(args.metrics)
    ...
    scored_path = _resolve_path(args.scored_output)
    csv_path = _resolve_path(args.csv_output)
    md_path = _resolve_path(args.md_output)
...
def main() -> int:
    ...
    parser.add_argument("--metrics", default="eval/benchmark_metrics.jsonl")
    parser.add_argument("--manifest", default="eval/manifest.jsonl")
    parser.add_argument("--scored-output", default="eval/benchmark_scored.jsonl")
    parser.add_argument("--csv-output", default="eval/benchmark_summary.csv")
    parser.add_argument("--md-output", default="eval/benchmark_summary.md")
